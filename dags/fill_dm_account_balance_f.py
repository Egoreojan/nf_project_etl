...
PROCESS_NAME = 'FILL_DM_ACCOUNT_BALANCE_F'

def fill_balance_for_period(start_date_str, end_date_str):
    db = None
    logger = None
    ...
    try:
        with PostgresDB(os.getenv("DB_NAME"), os.getenv("DB_USER"), os.getenv("DB_PASSWORD"), os.getenv("DB_HOST"), os.getenv("DB_PORT")) as db:
            ...
            while current <= end_date:
                db.execute("CALL ds.fill_account_balance_f(%s);", (current,))
...
with DAG(
    ...
    fill_turnover = PythonOperator(
        task_id='fill_balance_for',
        python_callable=fill_balance_for_period,
        op_args=[
            "{{ params.start_date }}", 
            "{{ params.end_date }}"
        ]
